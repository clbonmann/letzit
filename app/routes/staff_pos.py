...
async def validate_owner(db, oid, rid):
    return (await db.execute(text("SELECT 1 FROM offers WHERE id=:oid AND store_id=:rid"), {"oid": oid, "rid": rid})).scalar()
...
async def verify_qrcode(offer_id: int, payload: RedeemRequest, db: AsyncSession = Depends(get_db_session), staff: dict = Depends(get_current_staff)):
    if not await validate_owner(db, offer_id, int(staff["store_id"])): raise HTTPException(404, "Oferta inválida.")
...
async def mark_no_show(payload: NoShowRequest, db: AsyncSession = Depends(get_db_session), staff: dict = Depends(get_current_staff)):
    ...
    row = (await db.execute(text(f"SELECT c.id, c.status, c.client_id, o.store_id FROM offer_claims c JOIN offers o ON o.id=c.offer_id WHERE c.{field}=:val"), {"val": val})).mappings().first()
    
    if not row or row.store_id != int(staff["store_id"]): raise HTTPException(404, "Reserva inválida.")
