...
class ClientLoginRequest(BaseModel):
    ...
    # Geo (Obrigatório para o App funcionar)
    lat: float
    lon: float
    accuracy: Optional[float] = 0.0
    
    # Token de Push (Obrigatório para Smart Batch)
...
async def request_verification_code(
    ...
    """
    [LEGADO/BACKUP] Gera um código de 6 dígitos para SMS manual.
    Útil se você não usar o Firebase Auth no frontend.
    """
    phone = payload.phone_e164.strip()
    
    # 1. Gera código (Fixo 111111 para testes locais)
    code = "111111" # Em produção: str(random.randint(100000, 999999))
...
async def login_manual(
    ...
    """
    [LEGADO/BACKUP] Valida o código gerado no /request-code e loga.
    Username = Telefone, Password = Código de 6 dígitos.
    """
    ...
    # 2. Busca ou Cria o Usuário (UPSERT simplificado)
    # Como é login manual, não temos Lat/Lon nem FCM aqui, usamos valores padrão
    ...
    query = text("""
        INSERT INTO public.users (
            ...
            NOW(), ST_SetSRID(ST_MakePoint(0, 0), 4326) -- Geo padrão (oceano)
        )
    ...
    # 4. Token
    access_token = create_access_token(subject=str(user.id), extra_claims={"type": "client"})
...
async def client_login_app(
    ...
    O Frontend já verificou o telefone. Aqui nós só registramos/atualizamos.
    """
    
    # Query corrige os nomes das colunas para bater com sua tabela users
    query = text("""
        INSERT INTO public.users (
            phone_e164, 
            created_at, 
