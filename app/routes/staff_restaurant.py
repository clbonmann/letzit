...
# Certifique-se que estes utilitários existem no seu projeto, conforme os arquivos originais:
...
class RestaurantUpdate(BaseModel):
    id: Optional[int] = None # Opcional para Staff (pega do token), Obrigatório para Admin se for editar outro
    ...
    logo_url: Optional[str] = None # Caso queira atualizar URL direta sem upload
...
async def list_my_restaurants(
    ...
    - Se INTERNAL_ADMIN: Lista TODOS os restaurantes (Registry).
    - Se CLIENT_ADMIN/Staff: Lista APENAS o restaurante do usuário logado.
    """
    ...
    if role == "INTERNAL_ADMIN":
        ...
        # Staff vê apenas o seu
        query = text(f"{base_sql} WHERE id = :rid")
        params = {"rid": rid}

    rows = (await db.execute(query, params)).mappings().all()
    
    # Se for staff e não achar nada, é um erro de integridade de dados
...
async def update_restaurant_details(
    ...
    # 1. Definição do ID a ser editado
    if role == "INTERNAL_ADMIN":
        if not payload.id:
            raise HTTPException(400, "Admin deve informar o 'id' do restaurante no payload.")
        rid = payload.id
    else:
        rid = int(staff["restaurant_id"])

    # 2. Carrega dados atuais para validação e merge
    cur = (await db.execute(
        text("SELECT id, cnpj FROM restaurants WHERE id = :rid"),
        {"rid": rid},
    )).mappings().first()
    ...
    if new_cnpj and len(new_cnpj) != 14:
        raise HTTPException(400, "CNPJ deve conter 14 dígitos numéricos.")
    ...
    try:
        row = (await db.execute(
            ...
            """),
            {
                "rid": rid,
                "name": payload.name,
                "cnpj": (new_cnpj if new_cnpj else None),
        ...
        await db.rollback()
        raise HTTPException(409, "CNPJ já registrado em outro restaurante.")
...
async def upload_restaurant_logo(
    ...
    """
    Faz upload da imagem para o Cloudinary e atualiza o banco.
    """
    restaurant_id = int(staff["restaurant_id"])

    # 1. Upload para Cloudinary (Service existente)
    try:
